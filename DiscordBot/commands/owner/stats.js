...
module.exports = {
  ...
  run: async (client, message) => {
    ...
    const myarray = [];
    message.client.guilds.cache.keyArray().forEach(async function (item) {
      const guildMember = message.client.guilds.cache.get(item).memberCount;
      myarray.push(guildMember);
    });
    const sum = myarray.reduce(function (a, b) {
      return a + b;
    });
