...
module.exports = {
  ...
  run: async(client, message, args) => {  
    ...
    if (mentionedMember.roles.highest.position >= message.member.roles.highest.position && message.author.id !== message.guild.owner.id) 
      return message.channel.send(`:x: | **You can't kick this member due to your role being lower than that member's primary role.**`);
    ...
    if (mentionedMember.kickable) {
      const embed = new MessageEmbed()
        ...
        .setColor(`RANDOM`)
        .setDescription(`**Member:** ${mentionedMember.user.username} - (${mentionedMember.user.id})**Reason:** ${reason || `None`}`);
